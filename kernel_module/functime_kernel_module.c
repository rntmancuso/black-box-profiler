...
struct gen_pool *pool_param;
//for using in on_each_cpu
/*struct tlb_arg
{
  struct vm_area_struct *tlb_vma;
...
struct params
{
  int *size;  //for size of vma_index and number of pages will be sent to kernel in page_addr[]
        struct vmas_params *vma_param;
        int *page_addr; //page indices should send to kernel 
        bool shouldSkip; //cacheable or noncacheable 
	pid_t pid;
};
struct params cp;
//long *p;
...
/*void flush_tlb_page_mod(struct vm_area_struct *vma, unsigned long uaddr)
{
...
static int cacheability_modifier (pte_t *ptep, pgtable_t token ,  unsigned long addr,void *data)
{
	...
	//struct tlb_arg tlb;
	/*struct tlb_args ta;
	ta.ta_vma = ((struct Data*)data)->vmas;
	ta.ta_start = addr;*/
	//printk ("\naddr is:%x and skip is:%d\n", addr,skip);
        for (i=0; i<cp.size[PAGE_LIST]; i++) //check whether current addr is in the list pf pages we want to skip
	{
	  ...
	  //printk("i: %d\n",i);
		for (; count_vma < mm->map_count; count_vma++)
		{
		  ...
		  //	printk("vm_start is:%x\n",data->vmas->vm_start);
			...
			if (count_vma == cp.vma_param[i].vma_index)
			{
			       (void) __mm_populate_mod(data->vmas->vm_start, data->vmas->vm_end - data->vmas->vm_start,1,task);
				data->page_addr = kmalloc(cp.size[PAGE_LIST]*sizeof(long),GFP_KERNEL);
				int j;
...
void get_vma (void)
{
	...
	for_each_process(task)
	{
		...
		if(task->pid == cp.pid)
		{
			...
			mm = task->mm;
			/*	printk("cp.vma_param[0].vma_index:%d, cp.page_addr[0]:%d\n", cp.vma_param[0].vma_index,cp.page_addr[0]);
...
void filling_params(void)
{
	...
	/*cp has a field size with useful data and cp.page_addr which so far has a user ptr which is useless in kernel                                                                     
	...
	printk("cp.size[VMA_LIST]: %d and cp.size[PAGE_LIST]: %d\n", cp.size[VMA_LIST],cp.size[PAGE_LIST]);
	//cp.size[VMA_LIST] = 1;

	int* temp_page_addr = cp.page_addr;
	cp.page_addr = kmalloc(cp.size[PAGE_LIST]*sizeof(long),GFP_KERNEL);
	if(copy_from_user(cp.page_addr,temp_page_addr, cp.size[PAGE_LIST]*sizeof(long))) return -EFAULT;
	//printk("cp.page_addr[0]: %d\n", cp.page_addr[0]);
	...
	struct vmas_params *temp_vma = cp.vma_param;
	cp.vma_param = kmalloc(cp.size[VMA_LIST]*sizeof(struct vmas_params),GFP_KERNEL);
	if(copy_from_user(cp.vma_param,temp_vma, cp.size[VMA_LIST]*sizeof(struct vmas_params))) return -EFAULT;
	//									       printk("cp.vma_param[0].vma_index:%d\n", cp.vma_param[0].vma_index);
}
			      ...
			      size_t count, loff_t *data)
{
	...
	{
		filling_params();
		//printk("\ncp.vma_param[0].vma_index: %d,%d and cp.page_addr[0]: %d\n",cp.vma_param[0].vma_index,cp.page_addr[0]);
		get_vma(); 
	}
