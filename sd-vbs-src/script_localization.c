...
void symbol_localization_two(I2D* sType,F2D *sData, F2D* gyro,F2D *randW,float M_STDDEV_GYRO,F2D* norm_gyro,F2D* angleAlpha,float gyroTimeInterval,F2D *STDDEV_GPSPos,F2D *pos,F2D *Opos,F2D *temp_STDDEV_GPSPos,F2D* vel,F2D *qConj, F2D* orgWorld,float STDDEV_GPSVel, float STDDEV_ODOVel,F2D* accl, float M_STDDEV_POS, float acclTimeInterval, float pi,int icount,F2D* w,F2D* gtemp,F2D* gravity,int n,float STDDEV_ACCL,float M_STDDEV_VEL, F2D* resultMat, int rows, int cols,F2D *ones, F2D *quatDelta,F2D* quat)
{
...
int main(int argc, char* argv[])
{
    ...
    while(1)
    {
        ...
        */
        ...
        start = photonStartTiming(); 
	//Gol
	symbol_localization_two(sType,sData,gyro,randW,M_STDDEV_GYRO,norm_gyro,angleAlpha,gyroTimeInterval,STDDEV_GPSPos,pos,Opos,temp_STDDEV_GPSPos,vel,qConj,orgWorld, STDDEV_GPSVel, STDDEV_ODOVel,accl,M_STDDEV_POS,acclTimeInterval,pi,icount,w,gtemp,gravity,n,STDDEV_ACCL,M_STDDEV_VEL,resultMat,rows,cols,ones,quatDelta,quat);
    /** Timing utils **/   
    endC = photonEndTiming();
