...
void symbol_svm(alphaRet* alpha,int N,  F2D* trn1,  F2D* trn2,int iter, F2D *a_result, F2D *b_result, F2D *Yoffset,F2D *Xtst,F2D *Ytst,
...
//result = fSetArray(Ntst,1,0);
   (*result_ptr) = fSetArray(Ntst,1,0); 
    ...
    //printf("adress of result_ptr:%xl test 9\n",*result_ptr);
    //result_ptr = &result;
...
int main(int argc, char* argv[])
{
    ...
    symbol_svm(alpha,N,trn1,trn2,iter,a_result,b_result,Yoffset,Xtst,Ytst,tst1,tst2,Ntst,&result,s,dim);

    //printf("addr of result:%lx test 10\n",result);
    /** Timing utils */
